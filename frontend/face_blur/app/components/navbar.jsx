...
export default function NavBar() {
  ...
  return (
   <header  className="  flex justify-between items-center py-4 px-4 lg:px-10">
        <Link className="text-3xl md:text-4xl lg:text-5xl font-light m-0">Fonn </Link>

        <nav className="hidden md:flex items-center gap-12">
            <Link  className=" hover:text-gray-300 z-50" to={"/"} >Home </Link>
            <Link className="  hover:text-gray-300 z-50 " to={"/upload"}>Upload </Link>
        ...
        <div id='mobileMenu' className='hidden fixed top-16 bottom-0 right-0 left-0 p-5 md:hidden z-40 bg-black/70 backdrop-blur-md'>
            <nav className='flex flex-col gap-6 items-center'>
                ...
                <Link className="  hover:text-gray-300 " to={"/upload"}>Upload </Link>
            </nav>
